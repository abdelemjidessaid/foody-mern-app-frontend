...
const CuisinesFilter = ({ onChange, selectedCuisines, isExpanded, onExpendedClick }: Props) => {
    ...
    <>
      <div className="flex justify-between items-center px-2">
        ...
        <div
          ...
          className="text-sm font-semibold mb-2 underline cursor-pointer text-bule-500"
        >
          Reset Filters
        </div>
      </div>
      <div className="space-y-2 flex flex-col">
        {cuisineList.slice(0, isExpanded ? cuisineList.length : 7).map((cuisine) => {
          ...
          return (
            <div className="flex">
              ...
              >
                {isSelected && <Check size={20} strokeWidth={3} className="mr-1" />}
                {cuisine}
              </Label>
            </div>
          );
        ...
        <Button onClick={onExpendedClick} variant="link" className="mt-4 flex-1">
          {isExpanded ? (
            <span className="flex flew-row items-center">
              View Less <ChevronUp />
            </span>
          ) : (
            <span className="flex flew-row items-center">
